...
import static com.infogain.utils.StringUtil.validateNonNullAndNonEmpty;
...
public final class MongoDBUtil {
  ...
  public MongoDBUtil connectClient(String connectionString) {
    ...
    Exceptions.wrap(e -> new RuntimeException("Failed to connect to MongoDB client", e))
        .run(
            () -> {
  ...
  public MongoDBUtil insertDocument(Document document) {
    ...
    Exceptions.wrap(e -> new RuntimeException("Failed to insert document", e))
        .run(
            () -> {
  ...
  public ObjectId getInsertedId() {
    return Exceptions.wrap(e -> new RuntimeException("Failed to retrieve inserted document ID", e))
        .get(
            () -> {
  ...
  public void closeClient() {
    Exceptions.wrap(e -> new RuntimeException("Failed to close MongoClient", e))
        .run(
            () -> {
              if (mongoClient != null) {
                mongoClient.close();
